...
The data cleaning process was as follows: 1. Loading in raw data. 2. Clean names: changing the names from upper case to lower-snake-case. 3. Filter data by only relevant chemicals/nutrients in the study. 4. Combine data frames into one. 5. Pivot dataset and remove NAs.

```{r}
read_csv(here("outputs", "bq1_clean.csv")) 
read_csv(here("outputs", "bq2_clean.csv"))
read_csv(here("outputs", "bq3_clean.csv"))
read_csv(here("outputs", "mpr_clean.csv"))
```
...
# Filter data by only relevant chemicals/nutrients in the study (k, no3_n, mg, ca, nh4_n). 
  ...
  #My filter data includes subsets like this one for every stream (bqq1,bq2,bq3,mpr). 
